...
```{r}
##### import the data #####
scRNA1 = read.delim("tasic_scRNAseq/tasic_training_b2.txt",header = FALSE, sep ='\t') 
scRNA <- scRNA1[,-1]
rownames(scRNA) <- scRNA1[,1]
scRNA = data.frame(t(scRNA))
rownames(scRNA) <- paste(rep("scRNA",dim(scRNA)[1]),1:dim(scRNA)[1],sep = "_")
  
seqFISH1 = read.delim("tasic_scRNAseq/seqfish_cortex_b2_testing.txt",header = FALSE, sep ='\t') 
seqFISH <- seqFISH1[,-1]
...
scRNA_label = read.table("tasic_scRNAseq/tasic_labels.tsv", sep = "\t")[,1]
seqFISH_label = read.table("tasic_scRNAseq/seqfish_labels.tsv", sep = "\t")[,3]
```
...
points(try.predict$variates[,1], try.predict$variates[,2], cex = 0.5)
legend("topleft",legend = c("scRNA","seqFISH"), pch = 1,pt.cex = c(1,0.5))

# comp 3&4
plotIndiv(try.train, ellipse = TRUE,comp = c(3,4),legend = TRUE, title = "sample plot by splsda" , ind.names = FALSE, type = classic, style = "graphics")
points(try.predict$variates[,3], try.predict$variates[,4], cex = 0.5)
legend("topleft",legend = c("scRNA","seqFISH"), pch = 1,pt.cex = c(1,0.5))

# comp 5&6
plotIndiv(try.train, ellipse = TRUE,comp = c(5,6),legend = TRUE, title = "sample plot by splsda" , ind.names = FALSE, type = classic, style = "graphics")
points(try.predict$variates[,5], try.predict$variates[,6], cex = 0.5)
legend("topleft",legend = c("scRNA","seqFISH"), pch = 1,pt.cex = c(1,0.5))
```
